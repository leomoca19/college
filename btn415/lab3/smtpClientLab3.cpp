    ...
    Config() { 
        ...
        while (getline(file, line)) {
            ...
            if(line == "sender") {
                ...
                std::cout << "Error reading from config.txt";
                std::terminate();
            }
        }
    }
...
int main() {
    ...
    // Sender credentials
    Config cfg;
    const std::string emailSender = cfg.sender;
    const std::string emailPassword = cfg.password;

    std::vector<std::string> receivers = cfg.receivers;
    ...
    try {
        ...
        // "To" header for multiple recipients
        std::ostringstream toHeader;
        toHeader << receivers.front();

        for (const auto& rcvr: std::views::drop(receivers, 1)) {
            toHeader << ',' << rcvr;
        }
        ...
        for (auto& rcvr: receivers)
            sendSMTPCommand(ssl, "RCPT TO:<" + rcvr + ">\r\n", "250");
